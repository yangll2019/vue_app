...
sql +=" AND captcha = md5(?)";
  //3:json
  pool.query(sql,[uname,captcha],(err,result)=>{
      if(err)throw err;
      if(result.length==0){
...
server.get("/showindex",(req,res)=>{
  ...
  var sql = "SELECT lid,price,title,subtitle,img_url,address,shelf_time FROM ticket_type LIMIT ?,?";
  var off = (pno-1)*ps;
  ps = parseInt(ps);
  pool.query(sql,[off,ps],(err,result)=>{
      if(err)throw err;
      obj.data = result;
      var sql = "SELECT count(*) AS c FROM ticket_type";
...
server.get("/cart",(req,res)=>{
  //1:参数(无参数)
  // console.log(req.session.uid);  //在服务器终端输出1
